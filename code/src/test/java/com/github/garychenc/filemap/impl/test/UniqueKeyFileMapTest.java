...
import com.github.garychenc.filemap.UniqueKeyFileMap;
...
public class UniqueKeyFileMapTest {
    ...
    public void testNormalUseCase() throws Exception {
        ...
        File storeDirFile = new File(forLocationFile.getParentFile(), "test-store");

        long startTime = System.currentTimeMillis();
        try (FileMap fileMap = new UniqueKeyFileMap(storeDirFile.getAbsolutePath(), "Test-1", true, false, 256)) {
            ...
            Assert.assertFalse(fileMap.add("TEST-KEY-1", "TEST-VALUE-1-001"));

            System.out.println(fileMap.size());
            Assert.assertEquals(5, fileMap.size());
            ...
            System.out.println(testValue1.getVersionNumber());
    ...
    public void testExtendStorageUseCase() throws Exception {
        ...
        long startTime = System.currentTimeMillis();
        try (FileMap fileMap = new UniqueKeyFileMap(storeDirFile.getAbsolutePath(), "Test-2", true, false, 256)) {
            System.out.println("Time Cost : " + (System.currentTimeMillis() - startTime));
        ...
        try (FileMap fileMap = new UniqueKeyFileMap(storeDirFile.getAbsolutePath(), "Test-2", true, false, 256)) {
            ...
            for (int i = 0; i < 255; i++) {
                Assert.assertFalse(fileMap.add("TEST-KEY-1-" + String.valueOf(i), "TEST-VALUE-0000001-" + String.valueOf(i)));
            }
    ...
    public void testMultiThreadNormalUseCase1() throws Exception {
        ...
        long startTime = System.currentTimeMillis();
        try (FileMap fileMap = new UniqueKeyFileMap(storeDirFile.getAbsolutePath(), "Test-3")) {
    ...
    public void testMultiThreadNormalUseCase3() throws Exception {
        ...
        long startTime = System.currentTimeMillis();
        try (FileMap fileMap = new UniqueKeyFileMap(storeDirFile.getAbsolutePath(), "Test-4")) {
            System.out.println("Create Finished. Time Cost : " + (System.currentTimeMillis() - startTime));
